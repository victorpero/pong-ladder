...
export default async function OrganizationPlayersPage({ organizationSlug }: { organizationSlug: string }) {
  const { organization } = await requireOrganizationUser(
    organizationSlug,
    organizationPath(organizationSlug, "players")
  );
          ...
          {users.length === 0 ? (
            ...
            users.map((user) => {
              ...
              return (
                <Link
                  key={user.id}
                  href={organizationPath(organizationSlug, "players", user.id)}
