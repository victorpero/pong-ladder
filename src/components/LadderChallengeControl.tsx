...
export function LadderChallengeControl({
  ...
  const labels = dictionary.ladder.challengeControl;
    ...
    case "outgoing":
      return (
        <span className="ladder-action bg-amber-50 text-warning">
          {labels.pending}
          <span className="sr-only">{t(labels.pendingWaiting, { player: opponentName })}</span>
        </span>
      );
    ...
    case "active":
      return (
        <Link
          ...
          aria-label={t(labels.activeAria, { player: opponentName })}
        >
          {labels.active}
        </Link>
      );
...
function ChallengeForm({
  ...
  const labels = useDictionary().ladder.challengeControl;
  ...
  return (
    <form action={action} className="relative">
      ...
      <ActionButton
        ...
        pendingLabel={labels.challenging}
        ariaLabel={t(labels.challengeAria, { player: opponentName })}
      />
  ...
  return (
    <form action={action} className="relative">
      ...
      <ActionButton
        ...
        label={labels.accept}
        pendingLabel={labels.accepting}
        ariaLabel={t(labels.acceptAria, { player: opponentName })}
      />
      <ActionError message={state.error} />
    </form>
  );
